...
public:
    ...
    {
        std::cout << "ParameterSlider begin gesture " << param.getName(20) << ", index "<< param.getParameterIndex() << "\n";
        param.beginChangeGesture();
    }
    
    void stoppedDragging() override
    {
        std::cout << "ParameterSlider end gesture " << param.getName(20) << ", index "<< param.getParameterIndex() << "\n";
...
class DemoParameterSlider   : public Slider, private Timer
{
public:
    DemoParameterSlider (AudioProcessorParameter& p) : Slider (p.getName (256)), param (p)
    {
        setRange (0.0, 1.0, 0.0);
        startTimerHz (30);
        updateSliderPos();
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DemoParameterSlider)
};
