...
{
public:
    ParameterSlider (AudioProcessorParameter* p) : Slider (p->getName(256)), parameter (p)
    {
    ...
    {
        std::cout << "ParameterSlider changed " << parameter->getName(20) << " (index: " << parameter->getParameterIndex() << ") ";
        std::cout << " to " << Slider::getValue() << "\n";
    ...
    {
        std::cout << "ParameterSlider started dragging " << parameter->getName(20) << " index: " << parameter->getParameterIndex();
    ...
    double getValueFromText (const String& text) override   { return parameter->getValueForText (text); }
    String getTextFromValue (double value) override         { return parameter->getText ((float) value, 1024); }
    
    void setParameter(AudioProcessorParameter* p) { parameter = p; }
